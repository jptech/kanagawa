//! Debug test for static if then branch error in control/loop.k
...
fn debug_loop_k() {
    ...
    // Find all StaticIfDecl nodes and print their detailed structure
    for node in parse.syntax_node().descendants() {
        if node.kind() == SyntaxKind::StaticIfDecl {
            let offset: usize = node.text_range().start().into();
            let line = src[..offset].matches('\n').count() + 1;

            eprintln!("\n=== StaticIfDecl at line {} ===", line);
            print_tree(&node, &src, 0, 3);
        }
    }
