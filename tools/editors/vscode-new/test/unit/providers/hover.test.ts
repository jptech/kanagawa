...
describe('HoverProvider', () => {
    ...
    describe('Symbol Markdown Building', () => {
        ...
        function buildSymbolMarkdownContent(sym: MockSymbolInfo): {
            codeBlock: string;
            hasDoc: boolean;
            scopeInfo?: string;
        } {
            ...
            return {
                codeBlock: summary,
                hasDoc: !!sym.docMarkdown,
                scopeInfo: sym.scopePath.length > 0 ? sym.scopePath.join('::') : undefined
            };
        }
        ...
        it('should use signature when available', () => {
            ...
            expect(content.codeBlock).to.equal('void push(T value)');
        });
        ...
        it('should include scope info when present', () => {
            ...
            const content = buildSymbolMarkdownContent(sym);
            expect(content.scopeInfo).to.equal('data.fifo::FIFO');
        });
        ...
        it('should indicate documentation presence', () => {
            ...
            const content = buildSymbolMarkdownContent(sym);
            expect(content.hasDoc).to.be.true;
        });
    });
